...
contains
    ...
    subroutine compute_gradient(func, hx, hy, hz, grad_x, grad_y, grad_z)
        implicit none
        real(dp), intent(in)  :: func(:,:,:)
        real(dp), intent(in)  :: hx, hy, hz
        real(dp), intent(out) :: grad_x(size(func,1), size(func,2), size(func,3))
        real(dp), intent(out) :: grad_y(size(func,1), size(func,2), size(func,3))
        real(dp), intent(out) :: grad_z(size(func,1), size(func,2), size(func,3))

        call d_x(func, hx, grad_x)
        call d_y(func, hy, grad_y)
        call d_z(func, hz, grad_z)
